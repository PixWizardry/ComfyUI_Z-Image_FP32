...
class ZImage(Lumina2):
    ...
    def process_unet_state_dict(self, state_dict):
        ...
        # 1. First pass: Rename keys and fix standard mappings
        for k, v in state_dict.items():
            new_k = k
            
            # Fix weird prefixes from Diffusers
            if "all_x_embedder.2-1." in new_k:
                new_k = new_k.replace("all_x_embedder.2-1.", "x_embedder.")
            if "all_final_layer.2-1." in new_k:
                new_k = new_k.replace("all_final_layer.2-1.", "final_layer.")
            
            # Fix Attention Normalizations
            if ".attention.norm_q." in new_k:
                new_k = new_k.replace(".attention.norm_q.", ".attention.q_norm.")
            if ".attention.norm_k." in new_k:
                new_k = new_k.replace(".attention.norm_k.", ".attention.k_norm.")
                
            # Fix Output projection naming
            if ".attention.to_out.0." in new_k:
                new_k = new_k.replace(".attention.to_out.0.", ".attention.out.")
            
            new_sd[new_k] = v

        # 2. Second pass: Concatenate Q, K, V into QKV
        final_sd = {}
        ...
        for k in keys:
            ...
            if "attention.to_q.weight" in k:
                # Identification
                base_key = k.replace("attention.to_q.weight", "attention.")
                
                # Fetch separate tensors
                q_key = base_key + "to_q.weight"
                k_key = base_key + "to_k.weight"
                v_key = base_key + "to_v.weight"
                
                if q_key in new_sd and k_key in new_sd and v_key in new_sd:
                    q = new_sd[q_key]
                    k_val = new_sd[k_key]
                    v_val = new_sd[v_key]
                    
                    # Concatenate on dim 0 (output features)
                    final_sd[base_key + "qkv.weight"] = torch.cat((q, k_val, v_val), dim=0)
                    
                    processed_keys.add(q_key)
                    processed_keys.add(k_key)
                    processed_keys.add(v_key)
                else:
                    print(f"Warning: Missing QKV parts for {base_key}")
            
            # If standard key, just copy
            elif "to_q" not in k and "to_k" not in k and "to_v" not in k:
                final_sd[k] = new_sd[k]
